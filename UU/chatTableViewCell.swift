    ...
    func rebuildUserInterface() {
        ...
        if (self.msgItem.user.username != "")
        {
           ...
           // self.avatarImage = UIImageView(image:UIImage(named:imageName))
            let name = thisUser.username != "" ? thisUser.username : "dsfdf"
            ...
            //userName.layer.borderColor = UIColor(white:0.0 ,alpha:0.2).cgColor
            //userName.layer.borderWidth = 1.0
            
            //别人头像，在左边，我的头像在右边
            let nameX =  (type == ChatType.someone) ? 2 : self.frame.size.width - 52
            
            //头像居于消息顶部
            let nameY:CGFloat =  0
            //set the frame correctly
            self.userName.frame = CGRect(x: nameX, y: nameY, width: 60, height: 30)
